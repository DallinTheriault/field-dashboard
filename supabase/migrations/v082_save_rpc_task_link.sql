-- v0.8.2 — estimator_save_estimate persists the optional line -> task link
--
-- Adds task_id to the line-item insert. Backward compatible: payloads
-- without the key insert NULL (the deployed pre-H2 app keeps working).
-- The guarded subselect nulls out any task that doesn't belong to this
-- job + tenant — a cross-job link can never be persisted, even if the
-- action-layer validation is bypassed.
--
-- Rollback: re-apply the v078 version of this function (task_id column
-- on estimate_line_items simply stays NULL).
...
BEGIN
  IF v_estimate_id IS NOT NULL THEN
    ...
    END IF;
    -- The task-ownership guard needs the job either way.
    SELECT job_id INTO v_job_id FROM public.estimates WHERE id = v_estimate_id;
  ...
  FOR rec IN SELECT value FROM jsonb_array_elements(COALESCE(p->'lines', '[]'::jsonb)) AS t(value) LOOP
    INSERT INTO public.estimate_line_items (
      ...
      is_hardware, sku, resolved_unit_price, hardware_markup, task_id
    ) VALUES (
      ...
      COALESCE((rec->>'is_hardware')::boolean, false),
      rec->>'sku',
      (rec->>'resolved_unit_price')::numeric,
      (rec->>'hardware_markup')::boolean,
      (SELECT t2.id FROM public.tasks t2
        WHERE t2.id = (rec->>'task_id')::bigint
          AND t2.job_id = v_job_id
          AND t2.client_id = v_client_id)
