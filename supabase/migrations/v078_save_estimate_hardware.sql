-- v0.7.8 — estimator_save_estimate writes hardware line fields
--
-- Status: applied to production via Supabase MCP. Repo copy for history.
-- Adds is_hardware / sku / resolved_unit_price / hardware_markup to the
-- line insert. Otherwise identical to v071.
...
BEGIN
  ...
  FOR rec IN SELECT value FROM jsonb_array_elements(COALESCE(p->'lines', '[]'::jsonb)) AS t(value) LOOP
    INSERT INTO public.estimate_line_items (
      ...
      is_hardware, sku, resolved_unit_price, hardware_markup
    ) VALUES (
